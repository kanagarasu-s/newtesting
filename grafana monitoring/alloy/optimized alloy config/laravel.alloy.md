...
loki.process "add_labels_laravel_logs" {
  ...
  // Combined regex for SQLSTATE and Exception
  stage.regex {
    expression = ".*(?:SQLSTATE\\[[0-9A-Z]+\\](?: \\[(?P<connection_code>[0-9]+)\\])?: [^:]+: (?P<error_code>[0-9]+)? (?P<connection_message>.*connections.*)?|(?P<exception_type>[A-Za-z0-9_\\\\]+Exception)).*"
  }

  stage.regex {
    expression = ".*(?:GET|POST|PUT|DELETE) (?P<api_call>/[A-Za-z0controller_method>[A-Za-z0-9_@]+).*"
  }
